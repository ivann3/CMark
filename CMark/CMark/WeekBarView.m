...
@interface WeekBarView()<CPTPlotDataSource>
...
-(void)barPlot:(CGFloat )length majorIntervalLength:(NSString *)len
{
    ...
    CPTGraphHostingView *hostingView=[[CPTGraphHostingView alloc]initWithFrame:CGRectMake(75, 0,300,220)];
    ...
    plotSpace.xRange=[CPTPlotRange plotRangeWithLocation:CPTDecimalFromCGFloat(0.0) length:CPTDecimalFromCGFloat(7.9) ];
    ...
    xAxis.orthogonalCoordinateDecimal=CPTDecimalFromString(@"0.5");
    xAxis.majorIntervalLength=CPTDecimalFromString(@"1");
    xAxis.minorTicksPerInterval=0.5;
...
//    自定义数据标签
    ...
    NSArray *xAxisLabels = [NSArray arrayWithObjects:[NSString stringWithFormat:@"%@月",[DataObject checkTime:@"MM"]],keyNames[0], keyNames[1], keyNames[2],keyNames[3], keyNames[4], keyNames[5],[NSString stringWithFormat:@"%@日",keyNames[6]] ,nil];
    ...
    yAxis.orthogonalCoordinateDecimal=CPTDecimalFromString(@"0.0");
...
//    lineStyle.lineColor=[CPTColor colorWithComponentRed:0.7 green:0.5 blue:0.9 alpha:1.0];
    ...
    CPTGradient *gradient=[CPTGradient gradientWithBeginningColor:color1 endingColor:color2 beginningPosition:0.4 endingPosition:1.0];
    gradient.angle=90.0;
...
//        NSLog(@"dict :%@",dict);
    NSNumber *today=[DataObject checkTime:@"dd"];
    
    int i=[today floatValue]-6+idx ;
//    NSLog(@"i:%d",i);
    
    float money=[[dict objectForKey:[NSString stringWithFormat:@"%d",i]] floatValue];
//    NSLog(@"money:%f",money);
