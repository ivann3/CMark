...
@interface MonthBarView()<CPTPlotDataSource>

@end

@implementation MonthBarView
{
...
-(void)barPlot:(CGFloat )length majorIntervalLength:(NSString *)len
{
    ...
    CPTGraphHostingView *hostingView=[[CPTGraphHostingView alloc]initWithFrame:CGRectMake(75, 0,310,220)];
    ...
    plotSpace.xRange=[CPTPlotRange plotRangeWithLocation:CPTDecimalFromCGFloat(0) length:CPTDecimalFromCGFloat(12.9) ];
    ...
    NSArray *xAxisLabels = [NSArray arrayWithObjects:[NSString stringWithFormat:@"月份"],keyNames[0],keyNames[1] ,keyNames[2] ,keyNames[3] ,keyNames[4] ,keyNames[5] ,keyNames[6] ,keyNames[7] ,keyNames[8] ,keyNames[9] ,keyNames[10] ,keyNames[11] ,nil];
...
//    NSLog(@"dict :%@",dict);
    ...
    float money=[[dict objectForKey:[NSString stringWithFormat:@"%lu",(unsigned long)idx+1]] floatValue];
